...
export class UploadPicture extends React.Component {
  ...
  ButtonPhoto = () => {
    ...
    this.props.fieldValues.pictureID = this.state.publicId,
      imagePreviewFirst = (<Image cloudName={CLOUD_NAME} publicId= {this.state.publicId} height='245' width='182'/>)
      }
    )}
}
  ...
  render() {
    if (this.state.publicId){
        imagePreview = (<Image cloudName={CLOUD_NAME} publicId= {this.state.publicId} height='245' width='182'/>)
    }
    else{
      imagePreview = (<div className="previewText"><StyleText>Cliquez pour ajouter une photo</StyleText></div>)
    }
    return (
      <button type ="button" onClick={this.ButtonPhoto}><AddPhoto>
        {imagePreview}
      </AddPhoto></button>
    );
  }
